...
- [1. 　class Eloop()　マルチタスクを実行するクラス](#1-class-eloopマルチタスクを実行するクラス)
  ...
  - [1.7. 　Suspender()　with構文を使ってturnの開始を一時的に停止させる（内部クラス）](#17-suspenderwith構文を使ってturnの開始を一時的に停止させる内部クラス)
- [2. 　class Button()　押しボタンの状態を取得するクラス](#2-class-button押しボタンの状態を取得するクラス)
  ...
  - [2.4. 　on\_action()　ボタン操作時のコールバック関数を設定する](#24-on_actionボタン操作時のコールバック関数を設定する)
  - [2.5. 　on\_pressed()　ボタン押下時のコールバック関数を設定する](#25-on_pressedボタン押下時のコールバック関数を設定する)
  - [2.6. 　on\_released()　ボタン解放時のコールバック関数を設定する](#26-on_releasedボタン解放時のコールバック関数を設定する)
  - [2.7. 　on\_held()　ボタン長押し時のコールバック関数を設定する](#27-on_heldボタン長押し時のコールバック関数を設定する)
  - [2.8. 付録](#28-付録)
    - [2.8.1. 　付録１　ボタンのモードの指定](#281-付録１ボタンのモードの指定)
    - [2.8.2. 　付録２　コールバック関数の呼び出し契機](#282-付録２コールバック関数の呼び出し契機)
...
### 1.7. 　Suspender()　with構文を使ってturnの開始を一時的に停止させる（内部クラス）
...
### 2.4. 　on_action()　ボタン操作時のコールバック関数を設定する
...
### 2.5. 　on_pressed()　ボタン押下時のコールバック関数を設定する
...
### 2.6. 　on_released()　ボタン解放時のコールバック関数を設定する
...
### 2.7. 　on_held()　ボタン長押し時のコールバック関数を設定する
