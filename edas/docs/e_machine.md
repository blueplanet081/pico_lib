...
- [1. 　class Eloop()　マルチタスクを実行するクラス](#1-class-eloopマルチタスクを実行するクラス)
  ...
  - [1.12. 　y\_sleep()　second秒が経過するまで yieldを繰り返すタスクジェネレータ（スタティックメソッド）](#112-y_sleepsecond秒が経過するまで-yieldを繰り返すタスクジェネレータスタティックメソッド)
  - [1.13. 　Suspender()　with構文を使ってturnの開始を一時的に停止させる（内部クラス）](#113-suspenderwith構文を使ってturnの開始を一時的に停止させる内部クラス)
- [2. 　class Button()　押しボタンの状態を取得するクラス](#2-class-button押しボタンの状態を取得するクラス)
...
#### 書式： <!-- omit in toc -->
    ...
    <float> = Eloop.idle_time()

<br>

### 1.12. 　y_sleep()　second秒が経過するまで yieldを繰り返すタスクジェネレータ（スタティックメソッド）
...
### 1.13. 　Suspender()　with構文を使ってturnの開始を一時的に停止させる（内部クラス）
