# e_machine モジュール　 <!-- omit in toc -->

## 目的 <!-- omit in toc -->

- e_module（シングルスレッドでのマルチタスクを実現するモジュール）の配下で押しボタン、ledなどの制御を行うモジュール
...
- Raspberry Pi Pico W
  ...
  - RPI_PICO_W-20240602-v1.23.0.uf2 以前は以下の問題がありました
    - Signalクラスのサブクラスから上位クラスのコンストラクタ呼び出した時に、キーワード引数がエラーになった

- Raspberry Pi Pico2 W
  - ファームウェアバージョン　RPI_PICO2_W-20250415-v1.25.0-preview.542.g9f3062799.uf2 以降
  - それ以前のファームウェアでは試験をしていません

- Raspberry Pi Pico / Pico2でも動作する可能性は高いですが、試験していません
...
- Pico / Pico W のルートディレクトリ、または libディレクトリに以下のファイルを格納する
  - e_module.py
  - e_machine.py
...
- e_machine.pyモジュールから必要なクラスを importする
...
- [1. 　class Eloop()　マルチタスクを実行するクラス](#1-class-eloopマルチタスクを実行するクラス)
  ...
  - [1.5. 　pause()　タスクを一時停止する](#15-pauseタスクを一時停止する)
...
- [2. 　class Button()　押しボタンの状態を取得するクラス](#2-class-button押しボタンの状態を取得するクラス)
  ...
  - [2.2. 　str\_reason()　状態コード(reason)に対応する、状態を表す文字列を返す](#22-str_reason状態コードreasonに対応する状態を表す文字列を返す)
...
- マルチタスク処理を行うイベントループを開始するスタティックメソッド
...
| 名前          | 型        | 内容                                                                      |
| ------------- | --------- | ------------------------------------------------------------------- |
| loop_interval | int, None | タイマーループの間隔を msec単位で指定する。省略時は規定値（100msec）、または現在の値 |
...
- マルチタスク処理を行うイベントループを停止するスタティックメソッド
...
- バックグラウンドで動作するタスクを生成し、イベントループに登録するスタティックメソッド
...
- タスクとして動作させるジェネレータオブジェクト、またはそのソースを以下では「タスクジェネレータ」と呼称する
...
- sync=True を指定すると、疑似スレッドを syncポイントで中断する。sync=Falseの場合は、その場で中断する。省略時は sync=False
- 中断した疑似スレッドは、start()メソッドで再開できる
...
- sync=True を指定すると、疑似スレッドを syncポイントで終了する。sync=Falseの場合は、その場で終了する。省略時は sync=False
...
- 押しボタンの状態を監視し、設定された機能を実行するクラス
- 本クラスの機能はすべて、バックグラウンドタスクで実行される

### 2.1. 　start()　ボタン情報を取得するタスクを開始する

- ボタン情報を取得するタスクを開始するクラスメソッド
...
| 名前       | 型         | 内容                                                                           |
| ---------- | ---------- | ------------------------------------------------------------------------------|
| pull_up    | Bool, None | pull_upかどうかの defaultを設定する。省略時は初期設定、または前回の設定を保持する   |
| name       | str        | 疑似スレッドの名前を設定する。省略時は "Bloop"が設定される                        |
| tracelevel | int        | トレース情報出力レベルを設定する（デバッグ用）。省略時は 0（出力しない）            |
| period     | int        | ボタン情報を取得する間隔を msec単位で指定する。省略時は初期設定（100msec）、または前回の設定を保持する |

- トレース情報出力レベルは [付録６ トレース情報出力レベル](#286-付録６トレース情報出力レベル) を参照

<br>

### 2.2. 　str_reason()　状態コード(reason)に対応する、状態を表す文字列を返す

- 状態コード(reason)に対応する、状態を表す文字列を返すクラスメソッド
- 状態コードと状態を表す文字列の対応は、[付録３ 状態コード](#283-付録３状態コード) を参照
...
- Pin、GPIO番号、または Bootselボタンに対して「押しボタン」オブジェクトを作成する
...
| 名前        | 型                       | 内容                                                       |
| ----------- | ------------------------ | ---------------------------------------------------------- |
| pin         | Pin, int, Bootsel_button | Buttonに割付ける Pinを指定する                             |
| name        | str                      | ボタンの名前（任意）、省略時は "noname"になる              |
| pull_up     | bool, None               | 押してLowなら pull_up=True、省略時は None（defaultを設定） |
| hold_time   | float, None              | HELD、REPEATED開始までのディレイ（秒）                     |
| repeat_time | float, None              | REPEATEDのインターバル（秒）                               |
| on_action   | function, None           | ボタンが操作されたときに呼び出すファンクション             |
| on_pressed  | function, None           | ボタンが押されたときに呼び出すファンクション               |
| on_released | function, None           | ボタンが離されたときに呼び出すファンクション               |
| on_held     | function, None           | ボタンが長押しされたときに呼び出すファンクション           |

<br>

- hold_time、repeat_time の指定によるボタンのモードについては、[付録１ ボタンのモードの指定](#281-付録１ボタンのモードの指定) を参照
- on_xxxx によるファンクションの呼び出し契機については、[付録２ ファンクションの呼び出し契機](#282-付録２ファンクションの呼び出し契機) を参照
- 引数 on_xxxx= によるファンクションの設定は、メソッド on_xxxx() によるものと同一だが、ここではファンクションの引数を設定することができない。ファンクションの引数まで設定する時はメソッドを使用するか、[Mu() Instant Closureクラス](#4-class-muinstant-closureクラス) を併用する 

<br>

### 2.4. 　on_action()　ボタンが操作されたときに呼び出すファンクションを設定する

- ボタンが操作されたときに呼び出すファンクションを設定する
