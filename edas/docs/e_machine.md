...
- [1. 　class Eloop()　マルチタスクを実行するクラス](#1-class-eloopマルチタスクを実行するクラス)
  ...
  - [1.3. 　idle\_time() 　通常のタスクが動いていない時間を取得する（スタティックメソッド）](#13-idle_time-通常のタスクが動いていない時間を取得するスタティックメソッド)
  - [1.4. 　create\_task()　新しいタスクを生成し、イベントループに登録する（スタティックメソッド）](#14-create_task新しいタスクを生成しイベントループに登録するスタティックメソッド)
  ...
  - [1.10. 　Suspender()　with構文を使ってturnの開始を一時的に停止させる（内部クラス）](#110-suspenderwith構文を使ってturnの開始を一時的に停止させる内部クラス)
- [2. 　class Button()　押しボタンの状態を取得するクラス](#2-class-button押しボタンの状態を取得するクラス)
...
### 1.3. 　idle_time() 　通常のタスクが動いていない時間を取得する（スタティックメソッド）
...
### 1.4. 　create_task()　新しいタスクを生成し、イベントループに登録する（スタティックメソッド）
...
### 1.10. 　Suspender()　with構文を使ってturnの開始を一時的に停止させる（内部クラス）
