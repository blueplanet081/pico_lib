...
- [1. 　class Edas()](#1-class-edas)
  ...
  - [1.11. 　idle\_time() 　通常のタスクが動いていない時間を取得する（クラスメソッド）](#111-idle_time-通常のタスクが動いていない時間を取得するクラスメソッド)
  - [1.12. 　\_get\_taskcount() 　動いているタスクの本数を返す（クラスメソッド）](#112-_get_taskcount-動いているタスクの本数を返すクラスメソッド)
  - [1.13. 　after()　指定時間（秒）後に functionを実行する（クラスメソッド）](#113-after指定時間秒後に-functionを実行するクラスメソッド)
  - [1.14. 　after\_ms()　指定時間（ミリ秒）後に functionを実行する（スタティックメソッド）](#114-after_ms指定時間ミリ秒後に-functionを実行するスタティックメソッド)
  - [1.15. 　y\_oneshot()　指定の関数を一回だけ実行するタスクジェネレータ（スタティックメソッド）](#115-y_oneshot指定の関数を一回だけ実行するタスクジェネレータスタティックメソッド)
  - [1.16. 　show\_edas()　動作中のタスクの一覧を表示するユーティリティメソッド（クラスメソッド）](#116-show_edas動作中のタスクの一覧を表示するユーティリティメソッドクラスメソッド)
  - [1.17. 　stop\_edas()　動作中のタスクを終了するユーティリティメソッド（クラスメソッド）](#117-stop_edas動作中のタスクを終了するユーティリティメソッドクラスメソッド)
  - [1.18. 　付録](#118-付録)
    - [1.18.1. 　付録１　トレース情報出力レベル](#1181-付録１トレース情報出力レベル)
    - [1.18.2. 　付録２　タスクの状態](#1182-付録２タスクの状態)
    - [1.18.3. 　付録３　タスクの性質](#1183-付録３タスクの性質)
...
- トレース情報出力レベルは [付録１ トレース情報出力レベル](#appendix01)を参照してください。
...
#### 書式： <!-- omit in toc -->
    ...
    <etask> = Edas(gen, name=None, previous_task=None, pause=False, terminate_by_sync=False, task_nature=BASIC):
...
- 「タスクの性質」の指定方法と意味は [1.18.3. 　付録３　タスクの性質](#1183-付録３タスクの性質)を参照してください。
...
### 1.11. 　idle_time() 　通常のタスクが動いていない時間を取得する（クラスメソッド）
...
### 1.12. 　_get_taskcount() 　動いているタスクの本数を返す（クラスメソッド）
...
### 1.13. 　after()　指定時間（秒）後に functionを実行する（クラスメソッド）
...
### 1.14. 　after_ms()　指定時間（ミリ秒）後に functionを実行する（スタティックメソッド）
...
### 1.15. 　y_oneshot()　指定の関数を一回だけ実行するタスクジェネレータ（スタティックメソッド）
...
### 1.16. 　show_edas()　動作中のタスクの一覧を表示するユーティリティメソッド（クラスメソッド）
...
### 1.17. 　stop_edas()　動作中のタスクを終了するユーティリティメソッド（クラスメソッド）
